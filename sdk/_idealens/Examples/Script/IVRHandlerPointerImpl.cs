...
namespace IVR
{
    public class IVRHandlerPointerImpl : IVRHandlerPointer
    {
...
#if !UNITY_EDITOR
            float neckToEyeHeight = IVRStruct.Instance.m_EyeInfo.eyeHeight - IVRStruct.Instance.m_EyeInfo.neckHeight;
            Vector3 pos = transform.localPosition;
            pos.y += neckToEyeHeight;
        ...
        void OnPress(ControllerButton type)
        {
            ...
            float time = Time.unscaledTime - startTime - 1;
            if (time < 1 && time > 0)
            {
                ...
                float radio = Mathf.Lerp(360, 0, time);
                //Debug.Log(radio);
