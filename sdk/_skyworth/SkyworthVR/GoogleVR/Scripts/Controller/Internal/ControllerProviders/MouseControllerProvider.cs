...
namespace Gvr.Internal {
  ...
  class MouseControllerProvider : IControllerProvider {
    ...
    private ControllerState state = new ControllerState( SvrControllerIndex.SVR_CONTROLLER_INDEX_RIGHT);
    ...
    private void UpdateOrientation() {
      ...
      Quaternion pitch = Quaternion.AngleAxis(deltaDegrees.y, Vector3.right);
            Matrix4x4 mmm =  Matrix4x4.TRS(Vector3.zero, yaw, Vector3.one) * Matrix4x4.TRS(Vector3.zero, pitch, Vector3.one);
            mmm *= Matrix4x4.TRS(Vector3.zero, state.orientation, Vector3.one);
#if UNITY_2017_3_OR_NEWER
            state.orientation = mmm.rotation;
#else
