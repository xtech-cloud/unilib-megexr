...
public class GvrControllerVisual : MonoBehaviour, IGvrArmModelReceiver
{
    ...
    public const string triggerButtonColorProperty = "triggerButtonColor";
    [SerializeField]
    private Color triggerButtonColor =
        new Color(197f / 255f, 197f / 255f, 197f / 255f, 1);
    ...
    public const  string CONTROLLERRENDERER = "controllerRenderers";
    [SerializeField]
    ...
    private int triggerColorId;
    private int triggerAlphID;
    ...
    private const float BATTERY_ALMOST_FULL = .205f;
    private const float BATTERY_MEDIUM = .3295f;
    private const float BATTERY_LOW = .455f;
    private const float BATTERY_CRITICAL = .605f;
    private const float BATTERY_HIDDEN = .765f;
    ...
    private void Initialize()
    {
        ...
        {
            controllerRenderers = GetComponentsInChildren<Renderer>(false);
        }
        ...
        triggerColorId = Shader.PropertyToID("_GvrTriggerColor");
        triggerAlphID = Shader.PropertyToID("_TriggerAlpha");

        materialPropertyBlock.SetColor(appButtonId, appButtonColor);
        materialPropertyBlock.SetColor(systemButtonId, systemButtonColor);
        materialPropertyBlock.SetColor(touchPadId, touchPadColor);
        materialPropertyBlock.SetColor(triggerColorId, triggerButtonColor);
        foreach (var item in controllerRenderers)
        {
    ...
    private void UpdateControllerState()
    {
        ...
        displayState.tiggerButton = GvrControllerInput.TriggerButton;
    ...
    private void OnVisualUpdate(bool updateImmediately = false)
    {
        ...
        {
            triggerDtate = Mathf.Max(0, triggerDtate - deltaTime / TOUCHPAD_RELEASE_DURATION_SECONDS);
            m_TriggerTarget.localRotation = Quaternion.identity;
        }

        materialPropertyBlock.SetFloat(triggerAlphID, triggerDtate);
    ...
    public void SetControllerTexture(Texture newTexture)
    {
        foreach (var item in controllerRenderers)
        {
            item.material.mainTexture = newTexture;
        }
